...
export function resolveSecScanScript(
  ...
  const scriptPath = join(root, 'scripts', 'aegis-scan.cjs');
...
export function resolveSecRemediateScript(
  ...
  const root = resolveGenieRoot(argv1, deps);
  const scriptPath = join(root, 'scripts', 'aegis-remediate.cjs');
  if (!deps.existsSync(scriptPath)) {
...
export function resolveSecFixScript(
  ...
  const root = resolveGenieRoot(argv1, deps);
  const scriptPath = join(root, 'scripts', 'aegis-fix.cjs');
  if (!deps.existsSync(scriptPath)) {
...
export function registerCommands(program: Command, deps: SecScanDeps = defaultDeps): void {
  // Aegis top-level commands (was `genie sec` → now flat `aegis`)
  program
...
`.trimStart(),
  ...
  const quarantine = program.command('quarantine').description('Quarantine lifecycle (list, gc)');
...
// ============================================================================
// Main entry — aegis CLI
// ============================================================================

import { Command } from 'commander';
import { readFileSync as readFileSync_ } from 'node:fs';
import { dirname as dirname_, join as join_, resolve as resolve_ } from 'node:path';
import { fileURLToPath } from 'node:url';

function readPackageVersion(): string {
  try {
    const here = dirname_(fileURLToPath(import.meta.url));
    const candidates = [
      join_(here, '..', '..', 'package.json'),
      join_(here, '..', 'package.json'),
      resolve_(here, '../package.json'),
    ];
    for (const p of candidates) {
      try {
        const pkg = JSON.parse(readFileSync_(p, 'utf8')) as { version?: string };
...
export function main(argv: string[] = process.argv): void {
  ...
  program
    .name('aegis')
    .description(
      'npm supply-chain incident-response CLI — scan, fix, remediate, verify',
    )
...
// Run only when invoked as CLI entry point (not when imported as a library).
